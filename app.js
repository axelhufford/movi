...
function drawLogoStrip(ctx, cx, top, scale, theme) {
  ...
  var idle = function(fx, label) {
    ...
    roundRectPath(ctx, fx + 24, 13, 14, 4, 1);
    ctx.fill();
    roundRectPath(ctx, fx + 24, 73, 14, 4, 1);
    ctx.fill();
  ...
  roundRectPath(ctx, 86, 5, 14, 4, 1);
  ctx.fill();
  roundRectPath(ctx, 86, 73, 14, 4, 1);
  ctx.fill();
  ctx.globalAlpha = 1;
...
async function renderShareCard(opts) {
  ...
  try {
    await Promise.all([
      ...
      document.fonts.load('italic 500 28px "Fraunces"', glyphs + SHARE_TAGLINE + SHARE_TAGLINE_TV + "My first ranked show movie TV 's ranking \xB7 ranked" + digits),
      document.fonts.load('400 26px "Instrument Sans"', glyphs + digits + "\xB7"),
